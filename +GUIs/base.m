    ...
    methods
        function obj = base(panelTitle, examplesDirectory)
            ...
            if ~exist('examplesDirectory', 'var')
                obj.examplesDirectory = ['examples',filesep];
            else
                obj.examplesDirectory = examplesDirectory;
            end
            
            addpath(genpath(pwd));
            % add all subfolders to path, so that GUI can be closed inside
            % sub folders
            
            scrsz = get(0, 'ScreenSize');
            obj.windowSize = scrsz;
            obj.wait = 0;
            % stops user input when software is busy
            
            % Fixed panel being not visible at start - alex
            ...
            obj.hMainWindow = figure('Position', [scrsz(3)*0.125 scrsz(4)*0.125 scrsz(3)*0.75 scrsz(4)*0.75],'Color', [1 1 1], 'KeyPressFcn', @(src,evt)(obj.handleKeyPress(src,evt)), 'CloseRequestFcn', @(src,evt)(obj.handleCloseRequest(src,evt)));
            
            try
                screensInOrder = GUIs.order;
                className = regexp(class(obj), '\.', 'split');
                screenID = strcmp(className{2}, screensInOrder);
                screenID = find(screenID == 1);
                if isempty(screenID)
                ...
                set(obj.hMainWindow,'name','VISTRA','numbertitle','off')
                % Custom Toolbar
                
                obj.hMainToolbar = uitoolbar( obj.hMainWindow );
                icon = imresize(imread(['+GUIs',filesep,'images',filesep,'icons',filesep,'home_48.png'],'BackgroundColor',[1 1 1]), [16 16]);
                ...
                icon = imresize(imread(['+GUIs',filesep,'images',filesep,'icons',filesep,'arrow_left_green_48.png'],'BackgroundColor',[1 1 1]), [16 16]);
                ...
                icon = imresize(imread(['+GUIs',filesep,'images',filesep,'icons',filesep,'arrow_right_green_48.png'],'BackgroundColor',[1 1 1]), [16 16]);
                ...
                icon = imresize(imread(['+GUIs',filesep,'images',filesep,'icons',filesep,'add_48.png'],'BackgroundColor',[1 1 1]), [16 16]);
                ...
                icon = imresize(imread(['+GUIs',filesep,'images',filesep,'icons',filesep,'search_48.png'],'BackgroundColor',[1 1 1]), [16 16]);
                ...
                icon = imresize(imread(['+GUIs',filesep,'images',filesep,'icons',filesep,'mouse_48.png'],'BackgroundColor',[1 1 1]), [16 16]);
                ...
                icon = imresize(imread(['+GUIs',filesep,'images',filesep,'icons',filesep,'refresh_48.png'],'BackgroundColor',[1 1 1]), [16 16]);
                uitoggletool(obj.hMainToolbar,'CData',icon,'TooltipString','Reset window', ...
                    'Separator','on', ...
                    'ClickedCallback', @(source, event)(obj.resetWindow(screensInOrder{screenID})));
        ...
        function createInputVideoSelectComboBoxAndText(obj, textPosition, position)
            ...
            obj.hInputImageSelect = uicontrol('Style', 'popupmenu', ...
                ...
                'Units', 'Normalized', ...
                'Position',position,...
                'String', {'Coastguard (I, 4 P) Quality 80' 'Foreman  (I, 4 P) Quality 50' 'Carphone  (I, 9 P) Quality 80'},...
                'Value', 1, ...
        ...
        function changeInput(obj, source)
            ...
            if size(imageRGB,3) == 1
                ...
                temp(:,:,2) = imageRGB;
                temp(:,:,3) = imageRGB;
                obj.inputMatrix = rgb2ycbcr(temp);
            else
        ...
        function resetWindow(obj, screenName)
            % ---------------------------------------------
            % Default callback for the reset button
            % ---------------------------------------------
            %
            if obj.wait == 0
                obj.wait = 1;
                h = waitbar(rand,['Loading ',screenName,' Example...']);
                close(obj.hMainWindow)
                clear obj
                GUIs.(screenName)();
                waitbar(1)
        ...
        function changeZoomMode(obj, source)
            ...
            if strcmp(get(source, 'State'), 'on')
                ...
                zoom('on');
                set(source, 'CData', imresize(imread(['+GUIs',filesep,'images',filesep,'icons',filesep,'search_48.png'],'BackgroundColor',[0 0 0]), [16 16]));
            else
                % off
                zoom('off');
                set(source, 'CData', imresize(imread(['+GUIs',filesep,'images',filesep,'icons',filesep,'search_48.png'],'BackgroundColor',[1 1 1]), [16 16]));
            end
        end
        ...
        function changePanMode(obj, source)
            ...
            if strcmp(get(source, 'State'), 'on')
                ...
                pan('on');
                set(source, 'CData', imresize(imread(['+GUIs',filesep,'images',filesep,'icons',filesep,'mouse_48.png'],'BackgroundColor',[0 0 0]), [16 16]));
            else
                % off
                pan('off');
                set(source, 'CData', imresize(imread(['+GUIs',filesep,'images',filesep,'icons',filesep,'mouse_48.png'],'BackgroundColor',[1 1 1]), [16 16]));
            end
        end
        ...
        function changeScreenMode(obj, source)
            ...
            if strcmp(get(source, 'State'), 'on')
                ...
                set(source, 'CData', imresize(imread(['+GUIs',filesep,'images',filesep,'icons',filesep,'add_48.png'],'BackgroundColor',[0 0 0]), [16 16]));
            else
                % off
                % Override and add your code here
                set(source, 'CData', imresize(imread(['+GUIs',filesep,'images',filesep,'icons',filesep,'add_48.png'],'BackgroundColor',[1 1 1]), [16 16]));
            end
        end
        ...
        function changeScreen(obj, screenName)
            ...
            % makes sure user cannot do multiple inputs at once
            if obj.wait == 0
                obj.wait = 1;
                % Intiate waitbar for user experience
                h = waitbar(1/3,['Loading ',screenName,' Example...']);
                pause(0.2)
                waitbar(2/3)
                close(obj.hMainWindow)
                clear obj
